...
import org.odk.collect.android.provider.ProjectsProvider.Companion.PROJECT_NAME_COLUMN_NAME
import org.odk.collect.android.provider.ProjectsProvider.Companion.PROJECT_UUID_COLUMN_NAME
...
class ProjectsProviderTest {
    ...
    fun `Query should return list of existing projects`() {
        ...
        contentResolver.query(ProjectsProviderAPI.CONTENT_URI, null, null, null, null).use { cursor ->
            ...
            assertThat(cursor.getString(cursor.getColumnIndex(PROJECT_UUID_COLUMN_NAME)), `is`("1"))
            assertThat(cursor.getString(cursor.getColumnIndex(PROJECT_NAME_COLUMN_NAME)), `is`("Project 1"))

            cursor.moveToNext()

            assertThat(cursor.getString(cursor.getColumnIndex(PROJECT_UUID_COLUMN_NAME)), `is`("2"))
            assertThat(cursor.getString(cursor.getColumnIndex(PROJECT_NAME_COLUMN_NAME)), `is`("Project 2"))

            cursor.moveToNext()

            assertThat(cursor.getString(cursor.getColumnIndex(PROJECT_UUID_COLUMN_NAME)), `is`("3"))
            assertThat(cursor.getString(cursor.getColumnIndex(PROJECT_NAME_COLUMN_NAME)), `is`("Project 3"))
        }
    }
