...
class CurrentProjectViewModelTest {
    ...
    fun setup() {
        ...
        `when`(currentProjectProvider.getCurrentProject()).thenReturn(Project("https://my-serverX.com", "username", "1234", "Project X", "X", "#cccccc"))
        currentProjectViewModel = CurrentProjectViewModel(currentProjectProvider)
    }

    @Test
    fun `Initial current project should be set`() {
        assertThat(currentProjectViewModel.currentProject.value!!, `is`(Project("https://my-serverX.com", "username", "1234", "Project X", "X", "#cccccc")))
    }

    @Test
    fun `setCurrentProject() should add new project to live data`() {
        val project = Project("https://my-serverY.com", "username", "1234", "Project Y", "Y", "#ffffff")
