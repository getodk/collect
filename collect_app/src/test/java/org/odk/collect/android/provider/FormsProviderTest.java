...
public class FormsProviderTest {
    ...
    @Test
    @Ignore("Not sure we need this")
    public void update_withCachePath_deletesCache() {
        Uri formUri = addFormsToDirAndDb("external_app_form", "1", "External app form");
        createFormFileInFormsDir("external_app_form", "1", "External app form renamed");
        ...
        ContentValues contentValues = new ContentValues();
        contentValues.put(JRCACHE_FILE_PATH, "newCache");
        ...
        try (Cursor cursor = contentResolver.query(formUri, null, null, null)) {
            ...
            cursor.moveToNext();
            assertThat(cursor.getString(cursor.getColumnIndex(JRCACHE_FILE_PATH)), is("newCache"));
        }
    }

    /**
     * Theory: this is attempting to prevent an edge case where the file path is changed, a new cache path
     * is generated and then that new cache path is deleted instead of the old one. This was based on
     * comments in the code that indicated ordering was important.
     */
    @Test
    @Ignore("Not sure we need this")
    public void update_withFormFilePath_andCachePath_deletesCache() {
        Uri formUri = addFormsToDirAndDb("external_app_form", "1", "External app form");
        File newFile = createFormFileInFormsDir("external_app_form", "1", "External app form renamed");
        String newHash = FileUtils.getMd5Hash(newFile);
        ...
        ContentValues contentValues = new ContentValues();
        contentValues.put(FORM_FILE_PATH, newFile.getAbsolutePath());
        contentValues.put(JRCACHE_FILE_PATH, "newCache"); // This won't be used as a new cache will be generated
