...
public enum StorageMigrationResult {
    ...
    public static String getResultMessage(StorageMigrationResult result, Context context) {
        ...
        switch (result) {
            case SUCCESS:
                message = context.getString(R.string.files_migration_completed);
                break;
            case NOT_ENOUGH_SPACE:
                message = context.getString(R.string.files_migration_not_enough_space);
                break;
            case FORM_UPLOADER_IS_RUNNING:
                message = context.getString(R.string.files_migration_form_uploader_is_running);
                break;
            case FORM_DOWNLOADER_IS_RUNNING:
                message = context.getString(R.string.files_migration_form_downloader_is_running);
                break;
            case MOVING_FILES_FAILED:
            case MIGRATING_DATABASE_PATHS_FAILED:
                message = context.getString(R.string.files_migration_failed);
                break;
        }
        return message;
    }
