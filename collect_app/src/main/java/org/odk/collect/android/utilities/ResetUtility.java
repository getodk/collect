...
public class ResetUtility {
        ...
        for (int action : resetActions) {
            switch (action) {
                ...
                case ResetAction.RESET_CACHE:
                    if (deleteFolderContents(StorageManager.getCachePath())) {
    ...
    private void resetPreferences(Context context) {
        ...
        boolean deletedSettingsFolderContest = !new File(StorageManager.getSETTINGS()).exists()
                || deleteFolderContents(StorageManager.getSETTINGS());

        boolean deletedSettingsFile = !new File(StorageManager.getOdkRoot() + "/collect.settings").exists()
                || (new File(StorageManager.getOdkRoot() + "/collect.settings").delete());
    ...
    private void resetForms() {
        ...
        File itemsetDbFile = new File(StorageManager.getMetadataPath() + File.separator + ItemsetDbAdapter.DATABASE_NAME);
