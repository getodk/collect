...
public class SelectMultiWidget extends SelectTextWidget implements MultiChoiceWidget {
    ...
    protected CheckBox createCheckBox(int index, boolean readOnlyOverride) {	// smap add readOnlyOverride
        ...
        checkBox.setFocusable(!getFormEntryPrompt().isReadOnly() && !readOnlyOverride);  // smap);
        checkBox.setEnabled(!getFormEntryPrompt().isReadOnly() && !readOnlyOverride);  // smap);
    ...
    protected void createLayout(boolean readOnlyOverride) {     // smap
        readItems();
        ...
        if (items != null) {
            ...
            for (int i = 0; i < items.size(); i++) {
                CheckBox checkBox = createCheckBox(i, readOnlyOverride);       // smap
