...
1. If you are an external contributor, make sure to discuss working on the issue with a maintainer who will then assign you to it - pull requests opened that aren't related to an assigned issue will be closed. If you're on the core team you can either assign yourself to an issue in [the backlog](https://github.com/orgs/getodk/projects/9/views/8) and move it to "in progress" or discuss adding an item to the backlog with the rest of the team.
...
- Use of AI tools to generate code must be disclosed by external contributors in the pull request description
- Commits that attribute an AI tool as the author or committer will not be accepted
- Code contributions generated by an AI without a human in the loop (that understands that code) will not be accepted
- Pull requests, issues or comments with text generated entirely by AI will be closed, deleted or ignored