...
public class InstanceChooserTabs extends TabActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        saved.addFlags(Intent.FLAG_ACTIVITY_FORWARD_RESULT);
        tabHost.addTab(tabHost.newTabSpec("tab1")
                .setIndicator(getString(R.string.saved_data))
                .setContent(saved));

        Intent completed = new Intent(this, InstanceChooser.class);
        completed.putExtra("status", "done");
        completed.addFlags(Intent.FLAG_ACTIVITY_FORWARD_RESULT);

        tabHost.addTab(tabHost.newTabSpec("tab2")
                .setIndicator(getString(R.string.completed_data))
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent intent) {
        Log.i("yaw", "instancechoosertabs "+requestCode+" "+resultCode+" "+intent);
